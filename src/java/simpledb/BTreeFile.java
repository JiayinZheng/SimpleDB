...
public class BTreeFile implements DbFile {
	 ...
	 * 
	 */
	private BTreeLeafPage findRightLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
			Field f) 
		...
		try{
			...
			BTreeInternalPage bTreeInternalPage = new BTreeInternalPage(pid,getPage(tid,dirtypages,pid,Permissions.READ_ONLY).getPageData(),keyField());
			Iterator<BTreeEntry> bTreeEntryIterator = bTreeInternalPage.reverseIterator();
			//用反向迭代器
			if(f==null){
				if(bTreeEntryIterator.hasNext()){
					return findRightLeafPage(tid,dirtypages,bTreeEntryIterator.next().getRightChild(),Permissions.READ_ONLY,f);
				}
				else{
					return (BTreeLeafPage) getPage(tid,dirtypages,pid,perm);
				}
			...
			while (bTreeEntryIterator.hasNext()){
				...
				if(f.compare(Op.LESS_THAN_OR_EQ,bTreeEntry.getKey())){
					return findRightLeafPage(tid,dirtypages,bTreeEntry.getLeftChild(),Permissions.READ_ONLY,f);
				}
				//要顺着找，直到找到合适的（不是二叉树！！！）
			}
			assert bTreeEntry != null;
			return findRightLeafPage(tid,dirtypages,bTreeEntry.getRightChild(),Permissions.READ_ONLY,f);
	 ...
	 * 
	 */
	BTreeLeafPage findRightLeafPage(TransactionId tid, BTreePageId pid, Permissions perm,
			Field f) 
					throws DbException, TransactionAbortedException {
		return findRightLeafPage(tid, new HashMap<PageId, Page>(), pid, perm, f);
	}
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		try{
			...
			if(f==null){
				if(bTreeEntryIterator.hasNext()){
					return findLeafPage(tid,dirtypages,bTreeEntryIterator.next().getLeftChild(),Permissions.READ_ONLY,f);
				}
				else{
					return (BTreeLeafPage) getPage(tid,dirtypages,pid,perm);
				}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		//原本在它后面的
		BTreeLeafPage newPage = (BTreeLeafPage) getEmptyPage(tid,dirtypages,page.pid.pgcateg());
		BTreeLeafPage oldNextPage=null;
		if(page.getRightSiblingId()!=null){
			//待拆分页有后续页
			oldNextPage = (BTreeLeafPage) getPage(tid,dirtypages,page.getRightSiblingId(),Permissions.READ_WRITE);
			oldNextPage.setLeftSiblingId(newPage.pid);
			newPage.setRightSiblingId(oldNextPage.pid);
		}
		else{
			newPage.setRightSiblingId(null);
		}
		page.setRightSiblingId((BTreePageId) newPage.getId());
		...
		int half = page.getNumTuples()%2==0?(page.getNumTuples()/2):(page.getNumTuples()/2+1);
		//留在原页上的个数
		int cur = 0;
		Tuple curTuple = null;
		while(cur<half){
			if(tupleReverseIterator.hasNext()){
				curTuple = tupleReverseIterator.next();
				page.deleteTuple(curTuple);
				newPage.insertTuple(curTuple);
				cur++;
			}
		}
		//错误写法：因为field是待插入的，而实际提升上去的应该是中位数BTreeEntry bTreeEntry = new BTreeEntry(field,bTreeLeafPage.pid,newPage.pid);
       ...
       newPage.setParentId(parentPage.pid);
...
//        updateParentPointers(tid,dirtypages,oldParent);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// some code goes here
        int moveNum = sibling.getNumTuples()-(sibling.getNumTuples()+page.getNumTuples())/2;
        int cur = 0;
		TupleDesc tupleDesc = page.td;
		...
		if(isRightSibling){
			curTuple = tupleIterator.next();//注意看图
		}
...
//		// some code goes here
		...
		while(cur<moveNum){
			...
			if(bTreeEntryIterator.hasNext()){
				curEntry = bTreeEntryIterator.next();
				//leftSibling.deleteKeyAndRightChild(curEntry);
				Field temp = curEntry.getKey();
				curEntry.setKey(parentEntry.getKey());
				parentEntry.setKey(temp);
				parentEntry.setLeftChild(leftSibling.pid);
				parentEntry.setRightChild(page.pid);
				parent.updateEntry(parentEntry);
				//偷梁换柱，只交换key内的值，不改变指针
				//leftSibling.updateEntry(curEntry);
				curEntry.setLeftChild(curEntry.getRightChild());
				leftSibling.deleteKeyAndRightChild(curEntry);
				Iterator<BTreeEntry> iterator2 = page.iterator();
				curEntry.setRightChild(iterator2.next().getLeftChild());
				//非常重要，要把指针改过来，否则无法插入
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		while(cur<moveNum){
			...
			if(bTreeEntryIterator.hasNext()){
				...
				curEntry.setKey(parentEntry.getKey());
				parentEntry.setKey(temp);
				parentEntry.setRightChild(rightSibling.pid);
				parentEntry.setLeftChild(page.pid);
				parent.updateEntry(parentEntry);
				//偷梁换柱，只交换key内的值，不改变指针
				//leftSibling.updateEntry(curEntry);
				curEntry.setRightChild(curEntry.getLeftChild());
				rightSibling.deleteKeyAndLeftChild(curEntry);
				Iterator<BTreeEntry> iterator2 = page.reverseIterator();
				curEntry.setLeftChild(iterator2.next().getRightChild());
				//非常重要，要把指针改过来，否则无法插入
				page.insertEntry(curEntry);
				page.updateEntry(curEntry);
		...
		//updateParentPointers(tid,dirtypages,leftSibling);
		updateParentPointers(tid,dirtypages,page);
//		int moveNum = rightSibling.getNumEntries()-(rightSibling.getNumEntries()+page.getNumEntries())/2;
//		int cur = 0;
//		Iterator<BTreeEntry> bTreeEntryIterator = rightSibling.iterator();
//		BTreeEntry curEntry = null;
//		while(cur<moveNum){
...
//				//偷梁换柱，只交换key内的值，不改变指针
//				rightSibling.deleteKeyAndLeftChild(curEntry);
//				curEntry.setLeftChild(page.reverseIterator().next().getRightChild());
//				page.insertEntry(curEntry);
//				page.updateEntry(curEntry);
//
//			}
...
//		updateParentPointers(tid,dirtypages,page);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage rightSib =
					(BTreeLeafPage) (this.getPage(
							tid,
							dirtypages,
...
//		}
	...
	public DbFileIterator reversedIterator(TransactionId tid) {
		return new BTreeReverseFileIterator(this, tid);
	}
...
class BTreeReverseFileIterator extends AbstractDbFileIterator {
	 ...
	 * @param tid - the transaction id
	 */
	public BTreeReverseFileIterator(BTreeFile f, TransactionId tid) {
		this.f = f;
		this.tid = tid;
	}
...
class BTreeReverseSearchIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		...
		if(ipred.getOp() == Op.EQUALS || ipred.getOp() == Op.GREATER_THAN
				|| ipred.getOp() == Op.GREATER_THAN_OR_EQ) {
			curp = f.findRightLeafPage(tid, root, Permissions.READ_ONLY, ipred.getField());
		}
		else {
			curp = f.findRightLeafPage(tid, root, Permissions.READ_ONLY, null);
		}
